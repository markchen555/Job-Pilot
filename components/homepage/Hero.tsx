...
function CallToAction({ compact = false }: CallToActionProps) {
  const buttonClass = compact ? "px-5 py-2.5" : "px-6 py-3";

  return (
    <div className="flex flex-col justify-center gap-3 sm:flex-row">
      <Link href="/login" className={`rounded-md bg-overlay text-center text-base font-medium text-accent-foreground transition-opacity hover:opacity-90 ${buttonClass}`}>
